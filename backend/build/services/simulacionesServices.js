...
export const obtieneSimulacionesPorCliente = async (cliente_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM simulaciones WHERE cliente_id = ?', [cliente_id]);
        return results;
    }
    catch (err) {
...
export const encuentraSimulacion = async (id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM simulaciones WHERE id = ? LIMIT 1', [id]);
        return results;
    }
    catch (err) {
...
export const agregarSimulacion = async (nueva) => {
    try {
        const validacion = simulacionSchema.safeParse(nueva);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query('INSERT INTO simulaciones (cliente_id, usuario_id, nombre_proyecto, descripcion, estado) VALUES (?,?,?,?,?)', [nueva.cliente_id, nueva.usuario_id, nueva.nombre_proyecto, nueva.descripcion, nueva.estado]);
        return results;
    }
    catch (err) {
...
export const borrarSimulacion = async (id) => {
    try {
        const [results] = await conexion.query('DELETE FROM simulaciones WHERE id = ?', [id]);
        return results;
    }
    catch (err) {
...
//Datos Techo
export const agregaDatosTecho = async (nuevo) => {
    try {
        const validacion = datosTechoSchema.safeParse(nuevo);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO datos_techo 
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [nuevo.simulacion_id, nuevo.geojson, nuevo.area_m2, nuevo.perimetro_m,
...
export const modificaDatosTecho = async (modificado) => {
    try {
        const validacion = datosTechoActualizarSchema.safeParse(modificado);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`UPDATE datos_techo SET 
            geojson = ?, area_m2 = ?, perimetro_m = ?, latitud = ?, longitud = ?, 
            tipo_techo = ?, angulo_inclinacion_deg = ?, azimut_deg = ?, 
            factor_sombra = ?, area_util_m2 = ? WHERE id = ?`, [modificado.geojson, modificado.area_m2, modificado.perimetro_m,
...
export const obtieneDatosTecho = async (simulacion_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM datos_techo WHERE simulacion_id = ? LIMIT 1', [simulacion_id]);
        return results;
    }
    catch (err) {
...
//Datos Geográficos
export const agregaDatosGeograficos = async (nuevo) => {
    try {
        const validacion = datosGeograficosSchema.safeParse(nuevo);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO datos_geograficos 
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, [nuevo.simulacion_id, nuevo.irradiacion_anual_kwh_m2, nuevo.irradiacion_diaria_promedio,
...
export const obtieneDatosGeograficos = async (simulacion_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM datos_geograficos WHERE simulacion_id = ? LIMIT 1', [simulacion_id]);
        return results;
    }
    catch (err) {
...
//Consumo Eléctrico
export const agregaConsumoElectrico = async (nuevo) => {
    try {
        const validacion = consumoElectricoSchema.safeParse(nuevo);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO consumo_electrico 
            ...
            VALUES (?,?,?,?,?,?,?,?)`, [nuevo.simulacion_id, nuevo.consumo_mensual_kwh, nuevo.consumo_anual_kwh,
...
export const modificaConsumoElectrico = async (modificado) => {
    try {
        const validacion = consumoElectricoActualizarSchema.safeParse(modificado);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`UPDATE consumo_electrico SET 
            consumo_mensual_kwh = ?, consumo_anual_kwh = ?, tarifa_kwh_mxn = ?, 
            costo_mensual_mxn = ?, tipo_tarifa = ?, numero_recibo = ?, periodo_facturacion = ? 
            WHERE id = ?`, [modificado.consumo_mensual_kwh, modificado.consumo_anual_kwh, modificado.tarifa_kwh_mxn,
...
export const obtieneConsumoElectrico = async (simulacion_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM consumo_electrico WHERE simulacion_id = ? LIMIT 1', [simulacion_id]);
        return results;
    }
    catch (err) {
...
export const agregaConfiguracionSistema = async (nueva) => {
    try {
        const validacion = configuracionSistemaSchema.safeParse(nueva);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO configuracion_sistema_solar 
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [nueva.simulacion_id, nueva.marca_panel, nueva.modelo_panel, nueva.potencia_panel_wp,
...
export const modificaConfiguracionSistema = async (modificada) => {
    try {
        const validacion = configuracionSistemaActualizarSchema.safeParse(modificada);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`UPDATE configuracion_sistema_solar SET 
            ...
            degradacion_anual_pct = ?, vida_util_anios = ? WHERE id = ?`, [modificada.marca_panel, modificada.modelo_panel, modificada.potencia_panel_wp,
...
export const obtieneConfiguracionSistema = async (simulacion_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM configuracion_sistema_solar WHERE simulacion_id = ? LIMIT 1', [simulacion_id]);
        return results;
    }
    catch (err) {
...
// ─── Resultados ────────────────────────────────────────────────
export const agregaResultadosCalculo = async (nuevo) => {
    try {
        const validacion = resultadosCalculoSchema.safeParse(nuevo);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO resultados_calculo 
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [nuevo.simulacion_id, nuevo.produccion_anual_kwh, nuevo.produccion_mensual_promedio_kwh,
...
export const obtieneResultadosCalculo = async (simulacion_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM resultados_calculo WHERE simulacion_id = ? LIMIT 1', [simulacion_id]);
        return results;
    }
    catch (err) {
