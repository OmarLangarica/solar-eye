...
export const obtieneClientesPorUsuario = async (usuario_id) => {
    try {
        const [results] = await conexion.query('SELECT * FROM clientes WHERE usuario_id = ?', [usuario_id]);
        return results;
    }
    catch (err) {
...
export const encuentraCliente = async (id) => {
    try {
...
export const agregarCliente = async (nuevo) => {
    try {
        const validacion = clienteSchema.safeParse(nuevo);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`INSERT INTO clientes 
            (usuario_id, nombre, apellido, email, telefono, direccion, ciudad, estado, codigo_postal, notas) 
            VALUES (?,?,?,?,?,?,?,?,?,?)`, [nuevo.usuario_id, nuevo.nombre, nuevo.apellido, nuevo.email, nuevo.telefono,
...
export const modificarCliente = async (modificado) => {
    try {
        const validacion = clienteActualizarSchema.safeParse(modificado);
        if (!validacion.success)
            return { error: validacion.error };
        const [results] = await conexion.query(`UPDATE clientes SET 
            nombre = ?, apellido = ?, email = ?, telefono = ?, 
            direccion = ?, ciudad = ?, estado = ?, codigo_postal = ?, notas = ? 
            WHERE id = ?`, [modificado.nombre, modificado.apellido, modificado.email, modificado.telefono,
...
export const borrarCliente = async (id) => {
    try {
        ...
        console.error('Error MySQL borrar cliente:', err);
